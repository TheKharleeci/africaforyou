...
const {
  ...
  INVALID_RESOURCE,
  RESOURCE_PASSWORD_RESET_FAIL_STATUS,
  RESOURCE_PASSWORD_RESET_FAIL
} = constants;
...
describe('Basic Constants Functions', () => {
  ...
  it('RESOURCE_DELETE_FAIL_STATUS', () => {
    ...
    expect(data).to.equal(`${randomResource}_DELETE_ERROR`);
  });
  it('INVALID_RESOURCE', () => {
    const data = INVALID_RESOURCE(randomResource);
    expect(data).to.equal(`Invalid ${randomResource}`);
  });
  it('RESOURCE_PASSWORD_RESET_FAIL_STATUS', () => {
    const data = RESOURCE_PASSWORD_RESET_FAIL_STATUS(randomResource);
    expect(data).to.equal(`${randomResource}_PASSWORD_RESET_FAIL`);
  });
  it('RESOURCE_PASSWORD_RESET_FAIL', () => {
    const data = RESOURCE_PASSWORD_RESET_FAIL(randomResource);
    expect(data).to.equal(`${randomResource} password reset failed`);
  });
});
